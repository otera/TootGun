...
interface HashtagPanelProps {
  ...
  onChange: (hashtags: string[], activeHashtags: string[]) => void
}

export default function HashtagPanel({ hashtags, activeHashtags, onChange }: HashtagPanelProps) {
    ...
    <div className="hashtag-panel">
      <div className="hashtag-list">
        {hashtags.map((tag) => (
          ...
          >
            <span>#{tag}</span>
            <span className="hashtag-remove" onClick={(e) => removeTag(tag, e)} title="削除">
              ×
            </span>
          </button>
        ))}
